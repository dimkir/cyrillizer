...
################################################################################
################################################################################
############ THIS SOURCE FILE SHOULD BE SAVED AS UTF-8 NO-BOM!!!! ##############
################################################################################
############character test: русские буквы éspáÑpOóúÚúUü 弋強 როლთ廋弅롰롘 
#######                     ľ,š,č,ť,ž
################################################################################

/** This file should be saved as UTF'8 NOBOM. It is using string constants which are 
 * to be in UTF8. You must see the characters above (from character test) correctly
 * (provided your viewer or editor supports and had recognized UTF'8 NOBOM encoding.
 * if you can't  see characters or they appear to be corrupted - maybe file encoding 
 * is corrupted.
 * 
 * TODO: 
 * I don't know whether behaviour of function trim() is UTF8 friendly or not. 
 * so far I have kept this function everywhere as in original (non-utf-8 version)
 * but in future, as my understanding grows - need to decide whether to keep it or not.
 * What i suspect is that there will be a lot of funny whitespace characters (like nbsp or smth similar
 * which may happen to be used in html pages. However I need also keep in mind that
 * this class has strict limitations on the characters allowed to be used in words.
 * @link see StringCaret::standartize() for more
 */
...
class SyllableSplitter2 implements ISyllableSplitter {
     ...
     * @throws (bubbles Exceptions from the StringCaret methods and from hte Character methods)
     *          namely: SyllableSplitterException
     */
    public function getSylalblesAndTonic($word) 
    {
        ...
        //$log->put($word);
        if ( substr($trimmed_word, 0, 3) == 'que') 
        {
     ...
     * ENCODING: despite it doesn't use mb_string functions, this method is only 
     * peforming byte-array operations. So this is why there's no need to convert it to mb.
     * It only words with bytes, not with w_chars.
     * 
    ...
    private function  _wordToArray($trimmed_word, $syllable_start_indices_array)
    {
       ...
       for ($i = 0 ; $i < count($syllable_start_indices_array) ; $i++)
       {
           ...
           $syllable = mb_substr($trimmed_word, $prevIndex, $lenOfSyllable );
       ...
       $curIndex = mb_strlen($trimmed_word); // now pointing at the first index outside word
       $syllable = mb_substr($trimmed_word, $prevIndex,  $curIndex - $prevIndex );
     ...
     * 
     * @param StringCaret $sCaret 
     * PRECONDITION: method is always called with the current character being 'y' or vowel
     */
    private function _shiftCaretOverNucleo($sCaret)
    {
        ...
        // TODO: asserting that precondition (see method comment above) is true
        if (!( $sCaret->curChar()->isVocal() || $sCaret->curChar()->equals('y') ))
        {
            throw new SyllableSplitterOperationNotSupportedException("Logic error: the precondition for the _shiftOverNucleo() method is that curChar() should be vowel or 'y'");
        }
...
//        }
        ...
        //  o el consonante
                ...
                if ( $sCaret->curChar()->isVocalCerradaTildada() )
                {
                    ...
                    // 
                    if ( $prevChar->isVocalCerrada() )  
                        // // hey: this actually can inculde any cerrada, including tildada
                        //TODO: maybe need to throw here invlalid letter combination exception, in case 
                        //there's two tildadas characters
                        // PUEDE ser `C`C 
                ...
                if ( $sCaret->curChar()->isVocalCerradaSinTilde() )
                {
                        if ( $sCaret->hasNextChar() )
                        {
                            if ( !$sCaret->nextChar()->isConsonant() )
                            {
                                // si hay 3 vocales enseguidas (provided that la segunda es C)
                                // se rompe el diptongo (y puede incluir el 'h'
                                if ( $hache )
                                {
                                    $sCaret->movePosition(-1);
                                }
...
//                        if ( !$sCaret->nextChar()->isConsonant() )
//                        {
//                            // se rompe el diptongo. es hiato
//                            if ( $hache )
//                            {
//                                $sCaret->movePosition(-2);
//                            }
//                            else
//                            {
//                                $sCaret->movePosition(-1);
...
//                        else
//                        {
//                            // 3rd character is consonant, so diptongo
...
//                        }
    ...
    private function _shiftCaretOverCoda($sCaret)
    {
        ...
        // POST CONDITION: now curChar is consonant
            ...
            // so possibly there may be more syllables made with our consonant
                ...
                // POST CONDITION: 
                //      curChar()  is consonant and 
                //      nextChar() is consonant and 
                //      nextChar() is NOT empty
                    ...
                    // POST CONDITION: 
                    //      curChar() is consonant, 
                    //      nextChar() is consonant, 
                    //      thirdChar() is valid (but we don't know yet if it is consonant or vowel)
    ...
    /**
     * Returns ONLY stringar with the syllables
     * @param String $word
     * @return Stringar
     * @throws (bubbles exceptions from getSylalblesAndTonic(String))
     */
     ...
     * @return String
     * @throws (bubbles exceptions from getSylalblesAndTonic()
     */
                    ...
                    implements IAbstractCharacterSurroundings
{
    ...
    const ALLOWED_CHARS = 'aábcdeéfghiíjklmnñoópqrstuúüvwxyz'; // should be WIDE_CHARS (this source file should be saved as UTF8)
    /**
     * Creates caret object to be able to traverse through the word. 
     * @param String $word  (spanish characters (as per ::standartize() function)
     *                 what if it is empty string?
     *                 what if it is string of spaces?
     * @throws SyllableSplitterException (in case word contains invalid characters or in case
     *                                      it's empty, or doesn't contain meaningful characters (eg. only whitespace or tabs)
     *                  
     */
    public function __construct($word_)
    {
    
        $this->original_word = $word_;
        //TODO: check for word validity: non empty, with valuable characters
        $this->word = self::standartize($word_); // throws exception in case there're not-allowed characters
        ...
        if ( $this->wordLen == 0)
        {
            throw new SyllableSplitterException(
                    __METHOD__. "::Error, empty word [$this->original_word] supplied to the caret, after standartization: [$this->word]");
        }
    ...
    public function getCharAtPos($pos)
    {
        ...
        return new Character($this, $this->word, $pos);
    }
    ...
    {
        // TODO: add standartization of other characters - (ie french tildas, etc)
        // TODO: trim maybe needs to be corrected as well? what's the story with trim in UTF-8 strings?
        //      there may be some fancy whitespaces 
        $word = trim($word);
        ...
        for ( $i = 0 ; $i < $mb_len ; $i++)
        {
            ...
            $w_char = mb_substr($word, $i, 1);
            $foundPos = mb_strstr($allowed_chars, $w_char);
            ...
            if ( $foundPos === false )
            {
                ...
                $byte_sequence = "HEX:".  strtoupper(bin2hex($w_char));
                throw new SyllableSplitterException("Error, in word [$word] 
                            found at widechar position [$i] unsupported character: 
                            [$w_char] byte sequence($byte_sequence). Only characters [$allowed_chars] are allowed.");
            }
        }
                ...
                implements IAbstractCharacterSurroundings
{
    ...
    /**
     * 
     * @param StringCaret $parentStringCaret //maybe just specific interface of the pointer head. not obligatory all the methods of it. w
     *                                            // we'll narrow it down later
     * @param String $wordWithCharacter   // thios is just 1char string (Characters should be STANDARTIZED!!!
     *                                                                                    // so only correct tildes and lowercase is accespted.
     */
    public function __construct($parentStringCaret, $wordWithCharacter, $posOfLetterInWordZeroBased) {
        $this->w_char = mb_substr($wordWithCharacter, $posOfLetterInWordZeroBased, 1);
        $this->sCaret = $parentStringCaret;
     ...
     * @param String $charlist_string  (Valid (non empty) substring: eg: 'kljmhng'  or 'yu' or 'y' 
     *                      TODO: what about empty charlist string??? 
     */
    public function isOneOfTheChars($charlist_string)
    {
        return self::_isOneOfTheCharsFromList($charlist_string, $this->getCh() );
    }
    
    public static function _isOneOfTheCharsFromList($charlist_string, $w_ch)
    {
        if ( mb_strlen($charlist_string) < 1)
        {
            throw new SyllableSplitterException("When verifying if character is one of the list, the list (charlist_string) cannot be empty.");
        }
        //$findPos = strstr($charlist_string, $this->getCh() );
        $findPos = mb_strstr($charlist_string, $w_ch );
    ...
    public function isConsonantExceptBut($stop_list)
    {
        ...
        $isInStopList = self::_isOneOfTheCharsFromList($stop_list, $this->getCh() );
...
//        $isInStopList =  ( $pos !== false);
     ...
     *  @param WString      (non empty, valuable chars)
     *  @returns Boolean 
     *  @throws SomeException
    ...
    public function pointsAt($substring)
    {
        ...
        $mb_len = mb_strlen($substring);
        $curCharIndex = $this->pos;
        for ($i = 0 ; $i < $mb_len ; $i++)
        {
            $w_ch = mb_substr($substring, $i, 1);
            if  ( $this->sCaret->getCharAtPos($curCharIndex + $i)->equals($w_ch) )
            {
